...
import PortfolioItemPage from '~/views/PortfolioItemPage/PortfolioItemPage.vue'
...
const portfolioTypes = Object.values(VALID_PAGE_TYPES)

const { data, error } = await useAsyncData('portfolio-alias', async () => {
...
function preparedData(response) {
    ...
    if (restId === ERROR_REST_ID || !portfolioTypes.includes(restId)) {
        throw404()
        return
    }

    if (VALID_PAGE_TYPES.PORTFOLIO_ITEM_PAGE) {
        portfolioItemPageStore.SET_PORTFOLIO_ITEM_PAGE_DATA(response)
    }
